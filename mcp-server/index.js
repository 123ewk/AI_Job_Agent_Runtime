#!/usr/bin/env node
// Chrome MCP Server —— AI Job Agent 的浏览器桥服务端。
//
// 职责：
//   - /mcp   ：MCP StreamableHTTP 端点（Bearer token 认证），给后端 Tool Adapter 用
//   - /ws    ：Chrome 扩展的 WebSocket 桥（首条消息 auth），给扩展 background 用
//   - /ping  ：健康检查（无认证）
//
// 通道设计（为什么不是 CDP）：
//   扩展经 chrome.scripting 在页面 MAIN world 执行脚本，不建立 chrome.debugger/CDP
//   调试会话 —— 这是绕过 BOSS 直聘反自动化检测（检测到 CDP 会关闭/回退页面）的关键。
...
// --- Config ---
const PORT = Number(process.env.MCP_PORT || process.env.BROWSER_MCP_PORT || 12307);
...
export function sendToExtension(method, params = {}) {
  return new Promise((resolve, reject) => {
    if (!extensionWs || extensionWs.readyState !== 1) {
      reject(new Error("Chrome Extension not connected. Open Chrome and configure the token in the extension popup."));
      return;
    }
...
function createSessionTransport() {
  const server = new McpServer({ name: "ai-job-agent-chrome-mcp", version: "1.0.0" });
...
// Health check (no auth) — extension_connected 供后端判定桥是否就绪
app.get("/ping", async () => ({
  status: "ok",
  extension: extensionWs?.readyState === 1,
}));
...
console.log(`[ai-job-agent-mcp-server] MCP server: ${mcpUrl}`);
console.log(`[ai-job-agent-mcp-server] WebSocket:  ${wsUrl}`);
...
if (isNew) {
  ...
  console.log(TOKEN);
  console.log(sep);
  console.log("\nMCP Client Config (backend .env)");
  console.log(`BROWSER_MCP_TOKEN=${TOKEN}`);
  console.log(sep);
} else {
  console.log(`[ai-job-agent-mcp-server] Token: ${TOKEN.slice(0, 8)}... (source: env or ~/.browser-mcp-secrets.json)`);
}

console.log("\n[ai-job-agent-mcp-server] Waiting for Chrome Extension...");
